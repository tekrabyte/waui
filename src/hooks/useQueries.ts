...
export const useGetCashflowCategories = () =>
  useQuery({
    queryKey: ['cashflow-categories'],
    queryFn: () => api.cashflowCategories.getAll(),
  });

export const useAddCashflowCategory = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: api.cashflowCategories.create,
    onSuccess: () => qc.invalidateQueries({ queryKey: ['cashflow-categories'] }),
  });
};
...
export const useUpdateCashflowCategory = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: { id: string } & any) => api.cashflowCategories.update(id, data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['cashflow-categories'] }),
  });
};
...
export const useDeleteCashflowCategory = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: string) => api.cashflowCategories.delete(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['cashflow-categories'] }),
  });
};
...
export const useCreateStandalonePromo = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['standalonePromos'] }),
  });
};
...
export const useUpdateStandalonePromo = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: { id: string } & any) => api.standalonePromos.update(id, data),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['standalonePromos'] }),
  });
};
